...
function buildTrade(
  ...
  return {
    ...
    bandState: position.bandState,
    crossCount: position.crossCount,
...
export function createPendingOrder(input: {
  ...
  return {
    ...
    score: signal.score,
    setup: signal.setup.kind,
    bandState: signal.bandState.state,
    crossCount: signal.crossCount,
    ...
    if (fillPrice !== null) {
      ...
      if (first !== undefined && first.qty > 0) {
        position = {
          ...
          score: pending.score,
          setup: pending.setup,
          bandState: pending.bandState,
          crossCount: pending.crossCount,
          entryTime: candle.openTime,
          filled: [first],
          pending: rest,
          stopPrice: plan.stopPrice,
          targetWidth: Math.abs(plan.takeProfitPrice - plan.averageEntryPrice),
          plannedRisk: plan.riskBudget,
