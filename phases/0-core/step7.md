...
2. **`RiskWarning`** — `plan.warnings`가 비어 있지 않으면 노란 배너로 전부 표시. 청산가가 손절가보다 가깝다는 경고는 빨간색으로 격상.

3. **`ScoreBreakdown`** — 8개 항목을 **전부** 표 형태로. 통과 ✓ / 실패 ✗ 와 `detail` 문자열을 그대로 보여준다. 점수 `n/8`을 헤더에 표시. 실패 항목을 숨기지 마라.

4. **`SignalPanel`** — 현재 지표값(EMA12, SMA20, BB 상·하단, BB 폭, ATR14, ADX14, 거래량/평균)과 마지막 확정봉 시각·종가·펀딩비.

5. **`CandleChart`** — 최근 100개 5분봉과 BB 상·중·하단, EMA12 오버레이. 진입가·손절가·익절가·청산가를 수평선으로 표시.

6. Claude 레짐 해설 — 버튼을 눌러야 `/api/commentary`를 호출한다. **자동 폴링하지 마라** (비용).
...
- 계좌 설정(자본금, 레버리지, 수수료율, 목표 수익률, 리스크 %, ATR 배수)을 조정하는 패널을 두고 `localStorage`에 저장한다. 기본값은 `CLAUDE.md`의 도메인 상수.
