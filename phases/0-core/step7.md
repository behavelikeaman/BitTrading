...
2. **수수료·유지증거금 실측 표시 (ADR-012)** — `/api/account-params`를 페이지 진입 시 1회 호출한다. `feeSource === 'measured'`면 실제 수수료율을, `'default'`면 "추정치 0.04%/side" 배지를 띄운다. 추정치일 때는 `OrderTicket`의 손익분기 승률 옆에 "수수료 추정 기준" 표시를 단다.
