...
- `/docs/ADR.md` (ADR-007 수수료·슬리피지·펀딩·청산 반영, ADR-013 목표는 R배수, ADR-014 슬리피지 실측, ADR-015 시장가·지정가 비교)
- `/CLAUDE.md`
...
1. **파라미터 패널** — 기간(from/to), 그리고 결과를 크게 바꾸는 값들을 조정 가능하게:
   ...
   - 레버리지, 수수료율, **슬리피지** (실측값이 있으면 기본값으로 채우고 "실측/추정" 표시)
   - **진입 방식 (`entryType`)** — `'market'`(기본) / `'limit'`, `limitValidBars`
...
2. **요약 카드** — 총 트레이드 수, 승률, 손익비(Profit Factor), 기대값(트레이드당 USDT), 최대낙폭(MDD), 최대 연속손실, **총 수수료**, 총 펀딩, **청산 횟수**, 최종 자본.
   ...
   - 총 수수료와 **총 슬리피지 비용**을 총이익 옆에 나란히 놓아 마찰 비중이 한눈에 보이게 한다.
   - 체결률(`fillRate`)과 슬리피지 실측/추정 여부를 함께 표시한다.
...
5. **시장가 vs 지정가 비교 (ADR-015)** — 버튼 하나로 같은 기간·같은 파라미터를 `entryType: 'market'`과 `'limit'` 양쪽으로 돌려 나란히 표시한다:
   - 총 신호 수(`signalCount`)는 같고 **체결률(`fillRate`)**이 다르다. 지정가의 체결률을 크게 보여준다.
   - 순이익, 승률, 총 마찰비용(수수료+슬리피지)을 나란히.
   - 지정가가 수수료는 싸지만 **놓친 트레이드** 때문에 순이익이 낮을 수 있다. 그 역선택 크기를 눈으로 보게 하는 것이 이 비교의 목적이다.

6. **손익분기 승률 비교** — 현재 설정의 실제 승률과 이론 손익분기 승률을 나란히 표시한다. 실제 승률이 손익분기 미만이면 경고. 이것이 "이 설정으로 돈을 벌 수 있는가"에 대한 직접적인 답이다.
...
- 총 슬리피지 비용과 체결률을 요약에서 빼지 마라. 이유: 시장가 매매에서 슬리피지는 수수료의 절반에 맞먹는 비용이고, 지정가 비교에서는 체결률이 곧 놓친 기회다.
