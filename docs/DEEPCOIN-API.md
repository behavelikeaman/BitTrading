...
CCXT 구현체(`requiredCredentials.password = true`)와 이 문서의 인증 규격은
`DC-ACCESS-PASSPHRASE` 헤더를 요구한다. 그런데 **위 발급 화면에는 패스프레이즈
입력란이 없다.** 서드파티 가이드는 "패스프레이즈는 거래소가 발급하는 값이 아니라
키 생성 시 사용자가 정하는 비밀번호"라고 설명하는데, 계정·UI 버전에 따라 그 단계가
없을 수 있다.

그래서 코드는 **패스프레이즈 없이도 호출을 시도한다** (빈 문자열로 전송). 미리
포기하면 "왜 실측이 안 되는가"에 답할 수 없기 때문이다. 판정은 거래소 응답으로
한다:

- 성공(code `0`) → 패스프레이즈가 필요 없는 키다. 그대로 쓴다.
- `401`/`403` 또는 서명 거부 코드 → 패스프레이즈가 필요하다. 키를 다시 만들면서
  입력란을 찾거나, 계정 보안 설정에서 API 비밀번호를 확인해야 한다.

결과는 대시보드 상단 배지에 한 줄로 뜬다 (`src/lib/credential-status.ts`).
